<div class="flex flex-row justify-end px-6 py-4" x-data="{
    ...
    likeStatus: {{ auth()->user()->likes->where('post_id', $post->id)->first()->like_status }},
     }">
    ...
    <div class="flex flex-col items-baseline">
        <button type="button" id="like" name="like"
            wire:click="updateLikeStatus({{ $post->id }}, {{ auth()->user()->id }}, 'like')"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" x-bind:class="{ 'active': likeStatus === 1}"
        ...
        <span wire:model="likes">{{ $post->likesCount() }}</span>
    </div>
    <!-- Dislike -->
    <div class="flex flex-col items-baseline">
        <button type="button" id="dislike" name="dislike"
            wire:click="updateLikeStatus({{ $post->id }}, {{ auth()->user()->id }}, 'dislike')"
            x-on:mouseover="showDislikePopover = true" x-on:mouseout="showDislikePopover = false"
            x-on:click="likeStatus === -1 ? likeStatus = 0 : likeStatus == 0 ? likeStatus = -1 : likeStatus = 0"
            class="hover:text-blue-700">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" viewBox="0 0 24 24" stroke-width="1.5" x-bind:class="{ 'active': likeStatus === -1}"
                stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class=" absolute bottom-16 left-0 right-0 z-10 bg-white rounded-lg border h-fit w-fit p-4">
        <div class="flex flex-col items-center">
            ...
            @forelse ($post->positiveLikes as $positiveLike)
                <span>{{ $positiveLike->user->userProfile->username }}</span>
                @if ($loop->iteration == 3)
                    <span class="text-sm italic">and {{ $post->positiveLikes->count() - 3 }} more</span>
                    @break
                @endif
                @empty
...
<div x-show="showDislikePopover" class="relative">
    <div class=" absolute bottom-16 left-0 right-0 z-10 bg-white rounded-lg border h-fit w-fit p-4">
        <div class="flex flex-col items-center">
            ...
            @forelse ($post->negativeLikes as $negativeLike)
                <span>{{ $negativeLike->user->userProfile->username }}</span>
                @if ($loop->iteration == 3)
                    <span class="text-sm italic">and {{ $post->negativeLikes->count() - 3 }} more</span>
                @break
            @endif
            @empty
                <span class="text-sm">Nothing yet</span>
