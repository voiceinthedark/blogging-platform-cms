<div class="flex flex-row justify-end px-6 py-4" x-data="{
    ...
    likeStatus: {{ auth()->user()?->likes->where('post_id', $post->id)->first()->like_status ?? 0}} ?? 0,
    }">
    ...
    <div class="flex flex-col items-baseline">
        ...
        <span wire:model="dislikes">{{ $post->dislikesCount() }}</span>

    </div>

    <div x-show="showLikePopover" class="relative">
        <div class="absolute bottom-16 right-0 inline-block z-10 bg-white rounded-lg border h-fit w-fit p-4">
